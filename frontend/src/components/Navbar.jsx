...
const Navbar = () => {
  ...
  const { setShowSearch, getCartCount, token, setToken, user, setUser, navigate } = useContext(ShopContext);
  ...
  return (
    ...
    <div className='flex items-center justify-between py-5 font-medium'>
      ...
      <ul className='hidden sm:flex gap-5 text-sm text-gray-700'>
        ...
        <NavLink to='/membership' className={'flex flex-col items-center gap-1 relative'}>
          <p>MEMBERSHIP</p>
          {user?.membershipActive && <span className='absolute -top-3 -right-6 text-[10px] bg-black text-white px-1 py-0.5 rounded'>Plus</span>}
          <hr className='w-2/4 border-none h-[1.5px] bg-gray-700 hidden' />
        </NavLink>
        {user?.role === 'admin' && (
          ...
          {token && (
            <div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4 z-50'>
              ...
              <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-slate-100 text-gray-500 rounded text-sm'>
                <p className='truncate'>Hello, {user?.name?.split(' ')[0] || 'User'}</p>
                <hr />
